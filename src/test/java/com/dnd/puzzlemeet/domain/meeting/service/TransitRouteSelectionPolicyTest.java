...
class TransitRouteSelectionPolicyTest {
  ...
  @Test
  @DisplayName("현재 정책은 약속 시각과 무관하게 언제나 TMAP 공급자를 고른다")
  void alwaysSelectsTmap() {
    ...
    assertThat(policy.select(future)).isEqualTo(TransitRouteProviderType.TMAP);
    assertThat(policy.select(past)).isEqualTo(TransitRouteProviderType.TMAP);
  }
}
